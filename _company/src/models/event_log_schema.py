...
class EventLogSchema(BaseModel):
    ...
    """
    timestamp: datetime.datetime | None = Field(default=None, description="이벤트가 발생한 정확한 시간")
    event_type: str | None = Field(default=None, description="이벤트의 유형 (e.g., view, search, purchase)")
    category: str | None = Field(default=None, description="세부 카테고리 (e.g., product_page, payment, keyword)")
